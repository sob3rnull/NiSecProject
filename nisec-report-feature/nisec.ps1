...
function Show-Help {
    ...
    Write-Host "  report    AI-narrated HTML report (charts+tables+Gemini narrative, mock by default) -> evidence/"
...
switch -Regex ($Target) {
    ...
    "^correlate$" {
        # Pure Python — no Git Bash needed, unlike the targets above.
        python scripts/correlate.py @Rest
    }

    "^correlate-test$" {
        python -m unittest tests.test_correlate -v
    }
